...
CREATE TABLE public.observation_images (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  observation_id uuid NOT NULL,
  image_type text CHECK (image_type = ANY (ARRAY['lepidoptera'::text, 'plant'::text, 'general'::text])),
  image_url text NOT NULL,
  storage_path text,
  uploaded_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT observation_images_pkey PRIMARY KEY (id),
  CONSTRAINT observation_images_observation_id_fkey FOREIGN KEY (observation_id) REFERENCES public.observations(id)
);
