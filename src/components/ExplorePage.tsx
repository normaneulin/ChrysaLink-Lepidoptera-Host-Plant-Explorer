  ...
  const MapView = () => (
    <div className="h-[600px] bg-gray-100 rounded-lg flex items-center justify-center relative">
      <div className="text-center text-gray-500">
        ...
        <p>Map view with {filteredObservations.length} observations</p>
        <p className="text-sm mt-2">Interactive map integration (Leaflet/Mapbox) would be added here</p>
      </div>
        ...
        <div className="grid grid-cols-6 gap-2">
          {filteredObservations
            ...
            .map((obs) => (
              <div
                key={obs.id}
                className="h-8 w-8 bg-green-600 rounded-full flex items-center justify-center text-white text-xs cursor-pointer hover:scale-110 transition-transform"
                onClick={() => setSelectedObservation(obs)}
                title={`${obs.lepidoptera.species} at ${obs.location}`}
              >
                🦋
              </div>
            ))}
